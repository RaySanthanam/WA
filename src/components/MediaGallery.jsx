    ...
    <div className="flex flex-col h-full bg-[#111b21]">
      ...
      <div className="flex border-b border-[#222d34]">
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium ${activeTab === 'media' ? 'text-[#00a884] border-b-2 border-[#00a884]' : 'text-[#8696a0]'}`}
        >
          Media
        </button>
        <button
          onClick={() => setActiveTab('docs')}
          className={`flex-1 py-3 text-sm font-medium ${activeTab === 'docs' ? 'text-[#00a884] border-b-2 border-[#00a884]' : 'text-[#8696a0]'}`}
        >
          Docs
        </button>
      </div>
        ...
        {activeTab === 'media' ? (
          ...
          <div className="space-y-2">
            {docMessages.map((msg) => (
              <a
                key={msg.id}
                href={`/data/media/${msg.media}`}
                download
                className="flex items-center gap-3 p-3 bg-[#202c33] rounded-lg hover:bg-[#2a3942] transition-colors"
              >
