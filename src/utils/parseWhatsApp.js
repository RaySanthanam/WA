...
export function parseWhatsAppChat(chatText, yourName) {
  ...
  const patterns = [
    // Indian/EU format without brackets: 10/10/24, 2:27 am - Name: message
    /^(\d{1,2}\/\d{1,2}\/\d{2,4}),\s*(\d{1,2}:\d{2}(?::\d{2})?\s*(?:am|pm|AM|PM)?)\s*-\s*([^:]+):\s*(.*)$/i,
    // US format with brackets: [1/5/24, 2:30:15 PM] Name: message
    /^\[(\d{1,2}\/\d{1,2}\/\d{2,4}),\s*(\d{1,2}:\d{2}(?::\d{2})?\s*(?:AM|PM)?)\]\s*([^:]+):\s*(.*)$/i,
    // 24h format with brackets: [05/01/2024, 14:30:15] Name: message
...
function parseTimestamp(dateStr, timeStr) {
  try {
    ...
    // Use D/M/YY format (day/month/year) - common in India/Europe/WhatsApp exports
...
function formatTime(timeStr) {
  ...
  // e.g., "2:30:45 PM" -> "2:30 PM", "14:30:15" -> "14:30", "2:27 am" -> "2:27 am"
  const cleaned = timeStr.trim();
  // Remove seconds if present, keep am/pm
